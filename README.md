...
- **One-step import**: paste a DOI or arXiv ID → the plugin fetches full metadata from [Crossref](https://crossref.org) and [Semantic Scholar](https://semanticscholar.org), creates an Article note, stubs for all authors, and a Journal note.
...
3. Resolve each author: exact match → reuse; partial match or ORCID candidate → show a disambiguation modal asking *"Same person?"*.
4. Resolve the journal: exact match by full name or alias → reuse; new journal → create a Journal note.
5. Generate a BibTeX entry and create the Article (or Book) note at `Articles/Surname - YEAR - Title.md`.
